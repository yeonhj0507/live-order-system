...
-- 주문 상태 뷰: paid_at, order_screenshot, bundle_status로 주문 상태 판별
CREATE VIEW IF NOT EXISTS v_order_status AS
SELECT
    o.order_id,
    o.session_id,
    o.customer_id,
...
-- 세션별 매출 집계 뷰
CREATE VIEW IF NOT EXISTS v_session_revenue AS
SELECT
    s.session_id,
    s.session_date,
    s.session_title,
...
-- 미입금 주문 뷰
CREATE VIEW IF NOT EXISTS v_unpaid_orders AS
SELECT
    o.order_id,
    o.session_id,
    s.session_title,
